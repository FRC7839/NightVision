...
def sendValuesToCamera(robo_loc, cam_tol = "15"):
    tcp_port = 5800
    settings = []
...
def not_main(stdscr):
    ...
    while True:  
        all_menu_elements[0] = menuOneGetValues()
        ...
        # region arduino menü özel
        if current_menu == 2:
            if key != "button1" and key != "button0" and current_row == 0:
                robo_loc = curses_functions.get_robo_loc2(key, max_v)
                
            elif key != "button1" and key != "button0" and current_row == 1:
                cam_tol = key
                cam_tol = str(max_v - cam_tol)
        
            if key == "button0" and current_row == 2 and (not (robo_loc is None)):
                if not (cam_tol is None):
                    send_func_msg = sendValuesToCamera(robo_loc, cam_tol)
                else:
                    send_func_msg = sendValuesToCamera(robo_loc)
            
            all_menu_elements[2] = menuArduinoGetValues(send_func_msg, robo_loc, cam_tol)
            current_menu_elements = all_menu_elements[2][0]
            current_menu_status = all_menu_elements[2][1]
            
            
        
        # Eğer Arduino menüsünden çıkılırsa Server tekrar kontrol edilecek 
        else:
            send_func_msg = None
        #endregion
