...
servo_pin = board.get_pin("d:9:s")  # Arduino üzerine bağlı 9. pindeki servo tanıtıldı
gor = True  ## Benim gereksiz çözümlerim için
global servo_angle  # bunu elleme
servo_angle = 0  # Servo açısı main fonksiyonun başında sıfıra ayarlanıyor ama yine de sağlam olsun


def servo_add_angle(tunawashere):  # servonun belirli bir tarafa, belirli bir açıda döndürlmesi için
    servo_angle = servo_angle + tunawashere  # servonun açısına verilen açı ekleniyor
    servo_pin.write(servo_angle)


def servo_set_angle(angle):  # servonun belirli bir açıya döndürülmesi için
    servo_angle = angle
    servo_pin.write(servo_angle)
...
def main():
    servo_set_angle(0)  # servo en sağa alındı
    ...
    while True:
        ...
        if (success == False and servo_angle < 180 and gor == True):  # Eğer herhangi bir obje aktif olarak görülmüyorsa, servo en sola gelmediyse ve tunanın saçma değişkeni true ise
            try:                     # Eğer daha önce herhangi bir obje görüldüyse o objenin en son görüldüğü yöne dönmesi için
                if (y_error > 10 and servo_angle > 0):  # Herhangi bir obje görüldüğünde tanımlanan objenin ekranın tam ortasına olan uzaklığını belirten değişken y_error değişkeni
                    servo_add_angle(-1)     # Eğer y_error değişkeni tanımlanmadıysa daha önce bir obje görülmemiştir
                elif (y_error < -10):       # Ve eğer daha önce tanımlanmadıysa o değişkeni kullanmaya çalışmak bize aşağıda yazan hatayı döndürüyor
                    servo_add_angle(1)      # Bu kısmın varoluş amacı, bir objenin bizim onu takip edemeyeceğimizden daha hızlı bir biçimde ekrandan çıkması gibi durumlardır
                ...
                servo_add_angle(1)   # Servo bir derece sola döndürüldü

        elif success == False and (servo_angle == 180 or gor == False):  # Eğer herhangi bir obje aktif olarak görülmüyorsa ve servo en sola geldiyse "ya da" herhangi bir obje aktif olarak görülmüyorsa ve daha önce sağa doğru dönmeye başlanmışsa
            try:  # Eğer daha önce herhangi bir obje görüldüyse o objenin en son görüldüğü yöne dönmesi için
                if (success == True and y_error > 10 and servo_angle > 0):  # Yukarda ortaladığım dört satırda yazan şeylerin aynısı
                    servo_add_angle(-1)  # 2020 FRC 7839
                elif (success == True and y_error < -10 and servo_angle < 180):  # Tunapro1234
                    servo_add_angle(1)  # Neden yorum satırı bırakıyorum ki
            except UnboundLocalError:  # Eğer daha önce bir obje görülmediyse yavaş yavaş sola döndürmek için
                gor = False  # Sağa dönme işlemine başlatıldığını belirtmek için
                servo_add_angle(-1)  # buraları cidden incelemiyorsun değil mi
                if servo_angle == 0:  # Eğer en sağa gelindiyse
        ...
        try:
            ...
            if success == True and y_error > 10 and servo_angle > 0: # Eğer herhangi bir obje aktif olarak görülüyorsa, objenin orta noktası ekranın sağında kalıyorsa ve servo en sağda değilse
                servo_add_angle(-1) # Servoyu hafifçe sağa döndür

            elif success == True and y_error < -10 and servo_angle < 180: # Eğer herhangi bir obje aktif olarak görülüyorsa, objenin orta noktası ekranın solunda kalıyorsa ve servo en solda değilse
                servo_add_angle(1) # Servoyu hafifçe sola döndür
